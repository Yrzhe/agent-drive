 ...
 *
 * Every file resolution here MUST be scoped to the PUBLISHING bundle's owner (#65 audit).
 * Paths are per-owner unique since #30, so an unscoped path lookup on this unauthenticated
 * endpoint could resolve a DIFFERENT owner's unpublished row at the same path and serve it.
 */
...
publicBundlesRoutes.get(
  ...
  withErrorHandling(async (c) => {
    ...
    const [row] = await db
      ...
      .where(
        and(
          eq(files.path, fullPath),
          eq(files.isFolder, 0),
          isNull(files.deletedAt),
          bundle.ownerId ? eq(files.ownerId, bundle.ownerId) : undefined
        )
      )
      .limit(1);
